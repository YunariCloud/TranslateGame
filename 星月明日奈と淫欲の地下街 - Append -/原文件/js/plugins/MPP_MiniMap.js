...
(() => {
    ...
    Game_MinimapMarker.prototype.setMarker = function(markerStr) {
        ...
        if (match) {
            ...
            this._radius = Number(match[2]);
            this._index = Number(match[3]);
    ...
    PluginManager.registerCommand(pluginName, 'addMarker', args => {
    ...
    PluginManager.registerCommand(pluginName, 'removeMarker', args => {
        const tag = PluginManager.mppValue(args.tag);
        $gameMap.minimap.removeMarker(tag);
    });

    PluginManager.registerCommand(pluginName, 'removeMarkerXy', args => {
        const x = PluginManager.mppValue(args.x);
        const y = PluginManager.mppValue(args.y);
